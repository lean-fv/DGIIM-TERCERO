...
double AIPlayer::Poda_AlfaBeta(const Parchis &actual, int jugador, int profundidad, int profundidad_max, color &c_piece, int &id_piece, int &dice, double alpha, double beta, double (*heuristic)(const Parchis &, int)) const{
    ...
    if(jugador == actual.getCurrentPlayerId()){
        for(ParchisBros::Iterator it = hijos.begin(); it != hijos.end(); ++it){
            ...
            double valor=Poda_AlfaBeta(hijo, jugador, profundidad+1, profundidad_max, c_piece, id_piece, dice, alpha, beta, heuristic);
...
double AIPlayer::Heuristica(const Parchis &estado, int jugador){
    ...
    else
    {
        ...
        //Dados del jugador y del oponente
        vector<int> dados_jugador = estado.getAvailableNormalDices(jugador);
        vector<int> dados_oponente = estado.getAvailableNormalDices(oponente);

        //Puntuacion
        double puntuacion_jugador = 0;
        double puntuacion_oponente = 0;

        //Colas con prioridad para almacenar distancias en orden ascendente
        priority_queue<int> distancias;

        //Pesos
        vector<double> pesos = {2,1};

        //Auxiliar
        int val_distancia=0;

        //Comer
        //Pieza comida en el ultimo turno
        pair<color,int> eaten_piece = estado.eatenPiece();
        //Color de la ultima ficha movida
        color ultimo_color= get<0>(estado.getLastAction());

        if(ultimo_color == red || ultimo_color == yellow){
            if(eaten_piece.first == green || eaten_piece.first == blue) puntuacion_jugador += 10;
            else if(eaten_piece.first == yellow || eaten_piece.first == red) puntuacion_oponente += 10;
        }
        else{
            if(eaten_piece.first == yellow || eaten_piece.first == red) puntuacion_jugador += 10;
            else if(eaten_piece.first == green || eaten_piece.first == blue) puntuacion_oponente += 10;
        }
        ...
        //Energia
        puntuacion_jugador += PowerValue(estado.getPower(jugador));
        ...
        for (int i = 0; i < my_colors.size(); i++)
        {
            ...
            for (int j = 0; j < num_pieces; j++)
            {
                ...
                // Valoro positivamente que la ficha esté en casilla segura o meta.
                if (estado.isSafeBox(box)) puntuacion_jugador++;
                else if (box.type == goal) puntuacion_jugador += 10;

                //Valoro negativamente que la ficha este en casa
                else if(box.type == home) puntuacion_jugador-= 15;

                
                //Valoracion de la distancia a la meta
                val_distancia+=72-estado.distanceToGoal(c, j);
            }

            distancias.push(val_distancia);
            val_distancia=0;
        }

        //Ponderamos las distancias de las fichas
        for(int i=0; !distancias.empty(); i++){
            puntuacion_jugador += pesos[i]*distancias.top();
            distancias.pop();
        }
        ...
        //Energia
        puntuacion_oponente += PowerValue(estado.getPower(oponente));
        ...
        for (int i = 0; i < op_colors.size(); i++)
        {
            ...
            for (int j = 0; j < num_pieces; j++)
            {
                ...
                // Valoro negativamente que la ficha esté en casilla segura o meta.
                if (estado.isSafeBox(box)) puntuacion_oponente++;
                else if (box.type == goal) puntuacion_oponente += 10;
                
                //Valoro positivamente que la ficha este en casa
                else if(box.type == home) puntuacion_oponente-= 15;
        ...
        //Ponderamos las distancias de las fichas
        for(int i=0; !distancias.empty(); i++){
            puntuacion_oponente += pesos[i]*distancias.top();
            distancias.pop();
        }
...
double AIPlayer::PowerValue(int power) {

    int value=0;

    if(0<power<50) value = (7+power)/7;                 //Movimiento rapido
    else if(50<=power<60 || 70<=power<75) value = 50;   //Concha roja
    else if(60<=power<65) value = -25;                  //Boom
    else if(65<=power<70) value = 40;                   //Movimiento ultra rapido
    else if(75<=power<80) value = 60;                   //Movimiento bala
    else if(80<=power<85) value = -30;                  //Catapum
    else if(85<=power<90) value = 55;                   //Concha azul
    else if(90<=power<95) value = -30;                  //BoomBoom
    else if(95<=power<100) value = 40;                  //Movimiento estrella
    else if(power==100) value = -100;                   //Catapumchimpum
