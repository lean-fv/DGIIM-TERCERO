...
void funcion_filosofos( int id )
{
    ...
    while ( true )
    {
        ...
        MPI_Send(&msg,1,MPI_INT,id_contador,etiq_filosofo,MPI_COMM_WORLD);
    }
}
// ---------------------------------------------------------------------
...
void funcion_camarero(){
    ...
    while (true){
        ...
        if (estado.MPI_TAG==etiq_sentarse){
            ...
            MPI_Send(&valor,1,MPI_INT,id_contador,etiq_camarero,MPI_COMM_WORLD);
        }
    }
}
...
void funcion_contador(){
    ...
    while (true){
        ...
        if (contador_filosofos < contador_camarero) etiq_aceptada=etiq_filosofo;
        else if (contador_camarero < contador_filosofos) etiq_aceptada = etiq_camarero;
        else etiq_aceptada=MPI_ANY_TAG;

        MPI_Probe(MPI_ANY_SOURCE,etiq_aceptada,MPI_COMM_WORLD,&estado);

        cout << "Se va a recibir un mensaje del proceso numero " << estado.MPI_SOURCE << endl;
        

        MPI_Recv(&msg,1,MPI_INT,MPI_ANY_SOURCE,etiq_aceptada,MPI_COMM_WORLD,&estado);

        if (estado.MPI_TAG==etiq_filosofo) contador_filosofos++;
        else contador_camarero++;
        
        cout << "Contador de los filosofos: " << contador_filosofos << endl;
        cout << "Contador del camarero: " << contador_camarero << endl;
...
int main( int argc, char** argv )
{
    ...
    if ( num_procesos == num_procesos_actual )
    {
        ...
        else if(id_propio==id_contador) //es el contador
            funcion_contador();
