...
const int
   ...
   n_prods               = 2, //Numero de productores
   n_cons                = 2, //Numero de consumidores
   id_buffer             = n_prods, //Identificador del buffer
   etiq_prod       = 0, //etiqueta_aceptada para productores
   etiq_cons       = 1, //etiqueta_aceptada para consumidores
...
void funcion_buffer()
{
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      if ( num_celdas_ocupadas == 0 )               // si buffer vacío
         ...
         etiqueta_aceptada = etiq_cons;                      // $~~~$ solo cons.
      else                                          // si no vacío ni lleno
         etiqueta_aceptada = MPI_ANY_TAG;                    // $~~~$ cualquiera
