...
void repostar(int num_coche, const int tipo_coche){


    cout << "Coche " << num_coche << " (tipo de combustible " << tipo_coche << ") comienza a repostar." << endl;

    this_thread::sleep_for( chrono::milliseconds( aleatorio<20,100>() ));

    cout << "Coche " << num_coche << " (tipo de combustible " << tipo_coche << ") termina de repostar." << endl;
...
void funcion_coche(int num_coche){
    ...
    while(true){
        ...
        cout << "Coche " << num_coche << " (tipo " << tipo_combustible << ") solicita repostar." << endl;
...
void funcion_gasolinera(){
    ...
    while(true){
        ...
        if (hay_mensaje>0){
            ...
            cout << "Gasolinera: procesando fin de repostaje del coche " << estado.MPI_SOURCE 
                 << " (tipo de combustible " << tipo_combustible << ")"<< endl;
        }
        ...
        for(int tipo_comb=0; tipo_comb<num_combustibles; tipo_comb++){
            ...
            if(n_surtidores_libres[tipo_comb]>0){
                ...
                if(hay_mensaje_tipo>0){
                    ...
                    cout << "Gasolinera: procesando inicio de repostaje del coche " << estado.MPI_SOURCE 
                            << " (tipo de combustible " << tipo_combustible << ")"<< endl;
                }
...
int main( int argc, char *argv[] )
{
    ...
    if ( num_procesos_esperado == num_procesos_actual )
    {
        ...
        if ( id_propio == 0 ) 
            ...
            cout << "-----------------------------------------------------------------" << endl
            << "  Problema de la gasolinera " <<  num_combustibles << " surtidores (paso de mensajes MPI)." << endl
            << "------------------------------------------------------------------" << endl
            << flush ;
